#!/bin/bash
# build.sh - FoxAI Browser Linux build script
...
ESR_URL="https://ftp.mozilla.org/pub/firefox/releases/128.3.0esr/linux-x86_64/en-US/firefox-128.3.0esr.tar.bz2"
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD_DIR="${ROOT_DIR}/build-linux/work"
DIST_DIR="${ROOT_DIR}/release-linux"
...
log_step "FoxAI Browser Linux Build v${VERSION}"
...
fetch_firefox() {
    ...
    local installer="${BUILD_DIR}/firefox.tar.bz2"
    
    if [[ ! -f "${ROOT_DIR}/firefox-foxai/runtime/firefox" ]] || [[ ! -f "${BUILD_DIR}/firefox-extracted" ]]; then
        ...
        tar -xjf "${installer}" -C "${ROOT_DIR}/firefox-foxai/runtime" --strip-components=1
...
install_extensions() {
    ...
    local distro_ext="${ROOT_DIR}/firefox-foxai/runtime/distribution/extensions"
    mkdir -p "${distro_ext}"
    rm -f "${distro_ext}"/*
...
install_config() {
    ...
    cp "${CONFIG_DIR}/foxai.cfg" "${ROOT_DIR}/firefox-foxai/runtime/defaults/pref/foxai.cfg"
    ...
    echo "${pol}" > "${ROOT_DIR}/firefox-foxai/runtime/distribution/policies.json"
...
apply_branding() {
    ...
    local rcedit="${ROOT_DIR}/_salvage/rcedit-x64"  # Linux version needed
    ...
    # Note: rcedit is Windows-only, on Linux we skip binary patching
    log_warn "Skipping binary branding (rcedit is Windows-only)"
...
create_profile() {
    ...
    local firefox_bin="${ROOT_DIR}/firefox-foxai/runtime/firefox"
...
create_release() {
    ...
    local zip_name="FoxAI-Browser-v${VERSION}-linux-x86_64.tar.gz"
...
generate_sbom() {
    ...
    local sbom_path="${DIST_DIR}/FoxAI-Browser-v${VERSION}-linux-x86_64.sbom.json"
    ...
    cat > "${sbom_path}" <<EOF
{
  ...
  "metadata": {
    ...
    "component": {
      ...
      "hashes": [
        {"alg": "SHA-256", "content": "$(sha256sum "${DIST_DIR}/FoxAI-Browser-v${VERSION}-linux-x86_64.tar.gz" | cut -d' ' -f1)"},
        {"alg": "SHA-512", "content": "$(sha512sum "${DIST_DIR}/FoxAI-Browser-v${VERSION}-linux-x86_64.tar.gz" | cut -d' ' -f1)"}
      ]
    }
  },
  "components": []
...
sign_release() {
    if command -v minisign &> /dev/null && [[ -f "${HOME}/.minisign/minisign.key" ]]; then
        ...
        minisign -Sm "${DIST_DIR}/FoxAI-Browser-v${VERSION}-linux-x86_64.tar.gz" \
...
# Main build flow
main() {
    log_step "Starting FoxAI Browser Linux build v${VERSION}"
    ...
    echo -e "${GREEN}Release: ${DIST_DIR}/FoxAI-Browser-v${VERSION}-linux-x86_64.tar.gz${NC}"
}
