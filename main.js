...
const DICT = {
  en: {
    ...
    ad_h:"Ads never load. Trackers never phone home.", ad_sub:"uBlock Origin ships inside FoxAI Browser — not as an optional download, but as a locked part of the build.",
    ad1_t:"What it blocks", ad1_d:"Banner and video ads, third-party trackers, cryptominers, pop-ups and annoyances — filtered before they download, so pages load faster and use less data.",
    ad2_t:"Anti-fingerprint lists", ad2_d:"Tracking servers and fingerprinting scripts are cut at the network layer, backing up the browser's own RFP hardening.",
    ad3_t:"How it was added", ad3_d:"The signed uBlock Origin package (uBlock0@raymondhill.net.xpi) is bundled in the app's distribution folder and force-installed through the enterprise policy file (policies.json, ExtensionSettings → force_installed). It activates on first run, can't be silently removed by sites, and updates together with the browser.",
    cv_h:"Canvas looks identical everywhere.", cv_sub:"FoxAI Canvas Shield v3 mixes every canvas readout with a fixed key — all copies return the same pixels, so trackers can't tell you apart.",
    cv1_t:"What it guards", cv1_d:"The three exits sites use to fingerprint your GPU and fonts: getImageData, toDataURL and toBlob — on 2D canvas and OffscreenCanvas alike.",
    cv2_t:"How it works", cv2_d:"A content script runs before any page script (document_start, in every frame) and scrambles each pixel readout deterministically. Same input, same output, on every machine.",
    cv3_t:"Why deterministic", cv3_d:"No random noise — noise makes you stand out as the one with the weird canvas. Underneath sits full RFP hardening, and the fingerprint benchmark reads 100/100.",
    ...
    dl_rec:"Recommended", dl_ci:"CI artifact", dl_ci2:"CI artifact", dl_win_d:"Windows 10 · 11 · 12 auto (windows-latest)", dl_win_btn:"Download for Windows", dl_upd:"for updates", dl_lin_d:"Universal tar.gz — all distros (Ubuntu, Fedora, Arch, Debian)", dl_lin_btn:"Get Linux build (Actions)", dl_lin_hint:"All Linux · or from source: <code>build-linux/build.sh</code>.", dl_mac_d:"Universal .app — macOS auto (Sonoma → Tahoe 26 → 27)", dl_mac_btn:"Get macOS build (Actions)", dl_mac_hint:"Intel + Apple Silicon · or <code>build-macos/build.sh</code>.",
    ...
    faq8_q:"How does the AI work?", faq8_a:"The sidebar supports ChatGPT, Claude, Gemini or local Ollama. AI is off by default and the default mode runs on-device (KoboldCpp + Qwen). Page content is sent only after you click “Allow page access” — one action at a time, revocable, keys stored locally.",
    ...
    nav_contact:"Contact",
    contact_h:"Contact & Support", contact_sub:"Get help, report a bug, or just say hi — we reply within 24h. Auto-reply works even when we're offline.", contact_hint:"Auto-reply active 24/7 — human follow-up within a day.", contact_issue:"Open an issue", contact_gh_hint:"Source, releases & changelog",
    faq14_q:"How do I get support?", faq14_a:"Open an issue on GitHub, or email foxaibrowsersuppport@outlook.com / foxaibrowsersuppport@gmail.com — auto-reply even when offline, human reply within 24h.",
  ...
  tr: {
    ...
    ad_h:"Reklamlar hiç yüklenmez. İzleyiciler hiç eve telefon açmaz.", ad_sub:"uBlock Origin, FoxAI Browser'ın içinde gelir — isteğe bağlı bir indirme değil, yapının kilitli bir parçasıdır.",
    ad1_t:"Ne engeller", ad1_d:"Banner ve video reklamlar, üçüncü taraf izleyiciler, kripto madenciler, pencere ve rahatsızlıklar — indirilmeden süzülür, sayfalar daha hızlı açılır, daha az veri harcar.",
    ad2_t:"Parmak-izi karşıtı listeler", ad2_d:"İzleme sunucuları ve parmak izi betikleri ağ katmanında kesilir, tarayıcının kendi RFP korumasını destekler.",
    ad3_t:"Nasıl eklendi", ad3_d:"İmzalı uBlock Origin paketi (uBlock0@raymondhill.net.xpi) uygulamanın dağıtım klasörüne gömülüdür ve kurumsal politika dosyasıyla (policies.json, ExtensionSettings → force_installed) zorunlu kurulur. İlk açılışta etkinleşir, sitelerce sessizce kaldırılamaz, tarayıcıyla birlikte güncellenir.",
    cv_h:"Canvas her yerde aynı görünür.", cv_sub:"FoxAI Canvas Shield v3 her canvas okumasını sabit bir anahtarla karıştırır — bütün kopyalar aynı pikselleri verir, izleyiciler seni ayırt edemez.",
    cv1_t:"Neyi korur", cv1_d:"Sitelerin GPU ve fontlarını parmak-izlemek için kullandığı üç çıkış: getImageData, toDataURL ve toBlob — 2D canvas ve OffscreenCanvas dahil.",
    cv2_t:"Nasıl çalışır", cv2_d:"Bir içerik betiği her sayfa betiğinden önce çalışır (document_start, her çerçevede) ve her piksel okumasını belirli şekilde karıştırır. Aynı girdi, her makinede aynı çıktı.",
    cv3_t:"Neden belirli", cv3_d:"Rastgele gürültü yok — gürültü seni 'tuhaf canvaslı kişi' olarak öne çıkarır. Altta tam RFP sertleştirmesi var, parmak-izi testi 100/100 okur.",
    ...
    dl_rec:"Önerilen", dl_ci:"CI artifact", dl_ci2:"CI artifact", dl_win_d:"Windows 10 · 11 · 12 otomatik (windows-latest)", dl_win_btn:"Windows için indir", dl_upd:"güncellemeler için", dl_lin_d:"Evrensel tar.gz — tüm dağıtımlar (Ubuntu, Fedora, Arch, Debian)", dl_lin_btn:"Linux derlemesini al (Actions)", dl_lin_hint:"Tüm Linux · veya kaynaktan: <code>build-linux/build.sh</code>.", dl_mac_d:"Evrensel .app — macOS otomatik (Sonoma → Tahoe 26 → 27)", dl_mac_btn:"macOS derlemesini al (Actions)", dl_mac_hint:"Intel + Apple Silicon · veya <code>build-macos/build.sh</code>.",
    ...
    faq8_q:"AI nasıl çalışıyor?", faq8_a:"Kenar çubuğu ChatGPT, Claude, Gemini veya yerel Ollama destekler. AI varsayılan kapalıdır ve varsayılan mod cihaz-içi çalışır (KoboldCpp + Qwen). Sayfa içeriği sadece “Sayfaya erişime izin ver”e tıklayınca gönderilir — tek seferlik, geri alınabilir, anahtarlar yerel saklanır.",
    ...
    contact_h:"İletişim & Destek", contact_sub:"Yardım, hata bildirimi veya sadece merhaba — 24 saat içinde dönüş yapıyoruz. Çevrimdışıyken bile otomatik cevap aktif.", contact_hint:"7/24 otomatik cevap aktif — bir gün içinde insan dönüşü.", contact_issue:"Issue aç", contact_gh_hint:"Kaynak, sürümler ve değişiklik günlüğü",
    faq14_q:"Nasıl destek alırım?", faq14_a:"GitHub'da issue aç veya foxaibrowsersuppport@outlook.com / foxaibrowsersuppport@gmail.com’a yaz — çevrimdışıyken bile otomatik cevap gelir, 24 saat içinde yanıt.",
